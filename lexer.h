...
class Lexer
{
    ...
    public:
        ...
        Lexer(const std::string& source): current_pos(0), source(source), ungetted(), is_unget(false){}
        ...
        Token next();
        // Ungets a token - i.e. stores the token and on the next call to next, returns it
